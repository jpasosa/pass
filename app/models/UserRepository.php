...
class UserRepository
{
		...
		public function signup($input)
		{
				...
				if ( !is_null($input['image']) ) // Grabo la imágen en el servidor.
				{
					$filename 			= sha1(uniqid(mt_rand(), true));
					$complete_name 	= $filename . '.' . $input['image']->guessExtension();
					$move_file = $input['image']->move('uploads/avatar/', $complete_name);
					$user->image 		= $complete_name;
				}
